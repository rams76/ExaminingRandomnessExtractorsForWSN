...
void main()
{
	...
	unsigned char inData1[N];
	unsigned char inData2[N];
	unsigned char inData[2 * N];
	...
	unsigned int MAX=1600;
	unsigned int R;
	unsigned int C = 128;
	
	unsigned char DSuffix=0x06;
	unsigned long long int Input_Byte_Len=(2*N);
	unsigned long long int Output_Byte_Len=8;

	printf("Enter the filename1 to open without .bin \n");
	scanf("%s", inFilename1);
	//strcpy(outFilename, inFilename1);
	printf("Enter the filename2 to open without .bin \n");
	scanf("%s", inFilename2);

	strncat(inFilename1, ".bin", 4);
	strncat(inFilename2, ".bin", 4);
	//strncat(outFilename, "_bit.txt", 8);
	char outFilename[100] = "Multi_keccak.bin";
	// Open file 
	...
	while (ret1 == N  && ret2 == N) {
			...
			ret1 = fread(inData1, sizeof (char), N, inFile1);
			ret2 = fread(inData2, sizeof (char), N, inFile2);
			...
			inData[j] = inData1[j];
			inData[N + j] = inData2[j];
			
			}
			//printf("j=%d\n", j);
			Keccak(MAX - C, C, inData, Input_Byte_Len, DSuffix, outData, Output_Byte_Len);

			fwrite(outData, sizeof(char), 8 , outFile);
